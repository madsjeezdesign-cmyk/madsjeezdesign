...
import "./demo-commerce-premium.css";
...
function useScrollReveal(rootRef: React.RefObject<HTMLDivElement | null>) {
  useEffect(() => {
    ...
    if (!root) return;
    const els = root.querySelectorAll(".cm-scroll-reveal");
...
export function CommerceLanding({ slug }: Props) {
  ...
  const style = {
    ...
    "--cm-accent-dim": accentDim,
  } as React.CSSProperties;

  return (
    <div
      ref={rootRef}
      className="cm-premium relative min-h-screen overflow-x-hidden bg-[#0a0a0c] font-[family-name:var(--font-demo-b-commerce)] text-slate-100 antialiased"
      style={style}
    >
      <div className="cm-grid-bg pointer-events-none absolute inset-0 opacity-50" aria-hidden />
      <div className="cm-glow-orb -left-28 top-16 h-[28rem] w-[28rem]" aria-hidden />
      <div className="cm-glow-orb -right-24 bottom-32 h-72 w-72 opacity-25" aria-hidden />

      <header className="cm-glass fixed top-0 z-[100] w-full px-4 py-4 md:px-10 md:py-5">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-[family-name:var(--font-demo-h-commerce)] text-xl font-normal uppercase tracking-wide text-white md:text-2xl">
                {config.brand}
              </p>
              <p className="text-[9px] uppercase tracking-[0.04em] text-slate-500">{config.tradeLabel}</p>
            </div>
          </div>
          <nav className="hidden items-center gap-6 text-[10px] font-bold uppercase tracking-[0.04em] text-slate-400 lg:flex xl:gap-8">
            <a href="#planes" className="hover:text-[color:var(--cm-accent)]">
              Servicios
            </a>
            <a href="#features" className="hover:text-[color:var(--cm-accent)]">
              Ventajas
            </a>
            <a href="#stack" className="hover:text-[color:var(--cm-accent)]">
              Pagos
            </a>
        ...
        <div className="mx-auto grid w-full max-w-7xl items-center gap-12 px-4 md:px-10 lg:grid-cols-2 lg:gap-16">
          <div className="relative z-10">
            <div className="mb-6 inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.04em]">
              <span className="cm-pulse-dot h-2 w-2 rounded-full" style={{ background: config.accent }} />
              {config.heroKicker}
            </div>
            <h1 className="mb-6 font-[family-name:var(--font-demo-h-commerce)] text-5xl font-normal uppercase leading-[0.92] tracking-tight md:text-7xl lg:text-[5.5rem]">
              {config.heroTitle}
              <br />
            ...
            <div className="flex flex-wrap items-center gap-4">
              <button type="button" onClick={toggleQuote} className="cm-btn-primary px-8 py-4 text-[11px]">
                Pedir presupuesto
              </button>
              <a href="#planes" className="cm-btn-ghost inline-flex items-center gap-2 px-6 py-4">
                Ver paquetes <ArrowRight className="h-4 w-4" />
              </a>
            </div>
            <div className="mt-12 flex flex-wrap gap-3 text-[10px] font-bold uppercase tracking-[0.04em] text-slate-400">
          ...
          <div className="relative">
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-white/10">
              ...
              <div className="cm-glass absolute bottom-4 left-4 right-4 flex items-center justify-between rounded-xl p-4 md:bottom-6 md:left-6 md:right-6 md:p-5">
                <div>
                  <p className="text-[9px] uppercase tracking-[0.04em] text-slate-500">Estado del local</p>
            ...
            <div className="absolute -right-4 -top-4 hidden rounded-xl border border-white/10 bg-[#0c0c12] p-4 md:block">
              <Truck className="mb-2 h-5 w-5" style={{ color: config.accent }} />
              <p className="text-2xl font-black">4.9</p>
              <p className="text-[9px] uppercase tracking-[0.04em] text-slate-500">Clientes felices</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="planes" className="relative py-20 md:py-28">
        <div className="mx-auto max-w-7xl px-4 md:px-10">
          <div className="mb-14 text-center md:mb-16">
            <h2 className="font-[family-name:var(--font-demo-h-commerce)] text-4xl font-normal uppercase tracking-tight md:text-6xl">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {config.plans.map((plan) => (
              <article
                key={plan.name}
                className={`cm-plan-card cm-scroll-reveal rounded-2xl p-8 ${plan.popular ? "is-popular" : ""}`}
              >
                {plan.popular ? (
                  <span
                ...
                <p className="mt-6 font-[family-name:var(--font-demo-h-commerce)] text-4xl font-normal md:text-5xl">
                  {plan.price}
                ...
                <button
                  ...
                  className={`mt-10 w-full py-3 text-[10px] font-black uppercase tracking-[0.04em] ${
                    plan.popular ? "cm-btn-primary" : "cm-btn-ghost"
                  }`}
                >
                  Solicitar plan
                </button>
      ...
      <section id="features" className="border-y border-white/5 bg-[#0c0c12]/50 py-20 md:py-28">
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-4 md:px-10 lg:grid-cols-2">
          <div className="cm-scroll-reveal">
            <h2 className="mb-8 font-[family-name:var(--font-demo-h-commerce)] text-3xl font-normal uppercase md:text-5xl">
              Tu negocio <span style={{ color: config.accent }}>pro</span>
            </h2>
            <div className="grid gap-4 sm:grid-cols-2">
              {config.features.map((f) => (
                <div key={f} className="flex gap-3 rounded-xl border border-white/5 bg-white/[0.02] p-4">
                  <Check className="mt-0.5 h-4 w-4 shrink-0" style={{ color: config.accent }} />
                  <span className="text-sm text-slate-400">{f}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="cm-scroll-reveal relative aspect-video overflow-hidden rounded-2xl border border-white/10 lg:aspect-[4/3]">
            <Image src={v.a} alt="Vista del negocio" fill className="object-cover" sizes="50vw" />
            <div className="absolute inset-0 bg-gradient-to-tr from-[#0a0a0c]/85 to-transparent" />
          </div>
        </div>
      </section>
      ...
      <section id="stack" className="py-20 md:py-28">
        <div className="mx-auto max-w-7xl px-4 md:px-10">
          <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="font-[family-name:var(--font-demo-h-commerce)] text-3xl font-normal uppercase md:text-5xl">
                Pagos & <span style={{ color: config.accent }}>de pago</span>
              </h2>
              <p className="mt-2 text-sm text-slate-500">Opciones de cobro para conectar en producción</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-3">
            {config.payments.map((tool) => (
              <span
                key={tool}
                className="rounded-lg border border-white/10 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.04em] text-slate-400 transition-colors hover:border-[color:var(--cm-accent)] hover:text-white"
              >
                {tool}
              </span>
            ))}
      ...
      <section className="cm-scroll-reveal mx-4 mb-20 md:mx-10">
        <div
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-xl font-black uppercase tracking-[0.04em]">Atención al cliente</h3>
              <p className="mt-2 max-w-lg text-sm text-slate-400">
                Te acompañamos con pedidos, stock y presupuestos por WhatsApp para {config.tradeLabel}.
              </p>
            </div>
          </div>
          <Link href={`#lead-${slug}`} className="cm-btn-primary shrink-0 px-8 py-4 text-[10px]">
      ...
      <section id="faq" className="border-t border-white/5 py-20 md:py-28">
        <div className="mx-auto max-w-3xl px-4 md:px-10">
          <h2 className="mb-12 text-center font-[family-name:var(--font-demo-h-commerce)] text-3xl font-normal uppercase md:text-4xl">
            ...
            {config.faq.map((item, i) => {
              ...
              return (
                <button
                  ...
                  className="cm-glass w-full rounded-xl p-6 text-left"
                >
      ...
      <div
        className={`fixed inset-0 z-[200] transition-opacity ${quoteOpen ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"}`}
      ...
      >
        ...
        />
        <aside className="cm-glass absolute right-0 top-0 flex h-full w-full max-w-md flex-col justify-center p-8 md:p-12">
          <button type="button" onClick={closeQuote} className="absolute right-6 top-6 text-slate-500 hover:text-white">
            <X className="h-8 w-8" />
          </button>
          <p className="text-[10px] font-bold uppercase tracking-[0.04em]" style={{ color: config.accent }}>
            Cotización · {config.tradeLabel}
          </p>
          <h3 className="mt-4 font-[family-name:var(--font-demo-h-commerce)] text-3xl font-normal uppercase">
            Pedir presupuesto
          </h3>
          ...
          >
            <input
              ...
              className="w-full border-b border-white/10 bg-transparent py-3 text-sm outline-none focus:border-[color:var(--cm-accent)]"
            />
            <input
              type="email"
              placeholder="Email"
            ...
            <button type="submit" className="cm-btn-primary w-full py-4 text-[10px]">
              Enviar consulta
            </button>
          </form>
        </aside>
