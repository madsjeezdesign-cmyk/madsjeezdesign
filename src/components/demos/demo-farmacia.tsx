...
export function DemoFarmaciaLanding() {
  ...
  return (
    <div style={demoBodyStyle(SLUG)} className={`relative ${art.pageRoot}`}>
      ...
      <nav className="relative z-10 flex items-center justify-between border-b border-emerald-500/30 px-4 py-4 md:px-10">
        <span className={`flex items-center gap-2 text-xl font-semibold text-white ${h}`}>
      ...
      <DemoThemedHero
        ...
        kicker={
          <p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.3em] text-emerald-400">
        ...
        lead={
          <>
            Dispensación de medicamentos con receta, vacunatorios demo, venta de ortopedia liviana y cosmética dermocosmética con asesoría. Turno web para evitar fila: te avisamos por SMS cuando tu pedido está listo. Coberturas de obras sociales y prepagas según convenio vigente.
          </>
        }
        ctas={
          <>
      ...
      <DemoLongStory
        ...
        paragraphs={[
          ...
          "Nuestro equipo puede orientarte en genéricos bioequivalentes, horarios de antibióticos y interacciones básicas; las consultas complejas derivamos al prescriptor o a guardia. También tenemos servicio de compresas, alquiler de muletas y TENS demo con seña.",
        ]}
        kickerClass="text-emerald-400"
        titleClass="text-white"
        pClass="mt-4 text-sm leading-relaxed text-emerald-200/75 md:text-base"
        sectionClass="relative z-10 bg-emerald-950/80"
      />

      <section className="relative z-10 border-y border-emerald-500/20 px-4 py-14 md:px-10">
      ...
      <DemoStatsStrip
        ...
        sectionClass="relative z-10 border-y border-emerald-800/40 bg-black/20"
        cardClass={`p-6 ${art.cardShell}`}
        valueClass={`text-3xl font-black text-emerald-400 ${h}`}
      ...
      <DemoProcessSteps
        ...
        sectionClass="relative z-10 bg-emerald-900/25"
        titleClass="text-white"
      ...
      <DemoTestimonials
        ...
        sectionClass="relative z-10 bg-emerald-950/70"
        titleClass="text-white"
        cardClass={`p-6 ${art.cardShell}`}
      ...
      <DemoFaqList
        ...
        sectionClass="relative z-10 border-t border-emerald-800/30 bg-black/30"
        titleClass="text-white"
        qClass="font-bold text-white"
        aClass="mt-2 text-sm text-emerald-200/65"
      />

      <section className="relative z-10 bg-gradient-to-r from-emerald-600 to-teal-700 px-4 py-14 md:px-10">
        <div className="mx-auto flex max-w-5xl flex-col items-center justify-between gap-6 md:flex-row">
          <div className="flex items-center gap-4">
            <ShieldPlus className="h-14 w-14 text-white/30" />
            <div>
              <p className={`text-sm font-bold uppercase tracking-widest text-white/90 ${h}`}>Campaña demo</p>
              <p className={`mt-1 text-3xl uppercase text-white ${h}`}>20% ortopedia seleccionada</p>
            </div>
          </div>
      ...
      <DemoEnhancements
        ...
        sectionClass="relative z-10 border-y border-emerald-800/30 bg-emerald-950"
        titleClass="text-white"
        cardClass={`p-6 ${art.cardShell}`}
      ...
      <footer className="relative z-10 px-4 py-10 text-center text-xs text-emerald-800 md:px-10">
